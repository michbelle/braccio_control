/* Author: me file:: joy_braccio.cpp */
...
int main(int argc, char** argv)
{
  ...
  // Visualization per lavoro prima dell'effettivo utilizzo
  // ^^^^^^^^^^^^^
  //
  ...
  while(1)
  {
    //subscribe read the value from the joystic (1 value for time, the biggest one)
      // Setup
  // ^^^^^
  //
  // MoveIt! operates on sets of joints called "planning groups" and stores them in an object called
  ...
  std::copy(move_group.getJointModelGroupNames().begin(), move_group.getJointModelGroupNames().end(),
    ...
    geometry_msgs::Pose position_joy_buckup;

    position_joy=move_group.getCurrentPose().pose;
    
    position_joy=position_joy_buckup; // bisogna vedere se sono allo stessa allocazione di memoria come python
    //if value in array is max
    position_joy.position.x += 0.001;
    //position_joy.position.y += 0.001;
    //position_joy.position.z += 0.001;
  ...
  */
    ...
    /*
    if success is FALSE then led red
    if success is TRUE the led green
    GPIO for rasberri pi
    assegna valore di back up
    */
